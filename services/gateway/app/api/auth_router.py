...
async def proxy_auth_requests(request: Request, path: str) -> Response:
    """
    /auth/* 경로로 오는 모든 요청을 Auth 서비스로 프록시합니다.
    환경 변수 PROFILE에 따라 적절한 Auth 서비스 엔드포인트로 라우팅됩니다.
    
    - local: localhost:8080으로 라우팅
    - prod: auth_service:8080으로 라우팅
    """
    try:
        # 환경에 따른 Auth 서비스 URL 가져오기
        auth_service_url = settings.get_auth_service_url()
        
        logger.info(f"Routing auth request to: {auth_service_url} (profile: {settings.profile})")
